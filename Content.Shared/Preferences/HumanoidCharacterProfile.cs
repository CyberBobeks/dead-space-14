...
namespace Content.Shared.Preferences
{
    ...
    public sealed partial class HumanoidCharacterProfile : ICharacterProfile
    {
        ...
        public void EnsureValid(ICommonSession session, IDependencyCollection collection, string[] sponsorMarkings) // DS14-sponsors
        {
            ...
            // DS14-sponsor-species-start
            if (speciesPrototype.SponsorOnly && !sponsorMarkings.Contains(speciesPrototype.ID))
            {
                Species = SharedHumanoidAppearanceSystem.DefaultSpecies;
                speciesPrototype = prototypeManager.Index<SpeciesPrototype>(Species);
            }
            // DS14-sponsor-species-end
            ...
            // DS14-sponsor-tts-start
            if (voice is not null && voice.SponsorOnly && !sponsorMarkings.Contains(voice.ID))
            {
                Voice = SharedHumanoidAppearanceSystem.DefaultVoice;
            }
            // DS14-sponsor-tts-end
        }
        ...
        public ICharacterProfile Validated(ICommonSession session, IDependencyCollection collection, string[] sponsorMarkings) // DS14-sponsors
        {
            var profile = new HumanoidCharacterProfile(this);
            profile.EnsureValid(session, collection, sponsorMarkings); // DS14-sponsors
            return profile;
        }
